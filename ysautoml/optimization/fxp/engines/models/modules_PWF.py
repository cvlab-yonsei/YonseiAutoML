...
class grad_quant(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        if ctx.num_bits != 32:
            ...
            alpha = 2 ** (-ctx.clip_ratio-4)

            grad_max = grad_ori.abs().max()

            min_value = -grad_max * alpha
            max_value = grad_max * alpha

            grad_threshold = (grad_ori.abs() > max_value).float()
            ...
            grad_ori.add_(-min_value).div_(scale).add_(qmin)
            noise = grad_ori.new(grad_ori.shape).uniform_(-0.5, 0.5)
            grad_ori.add_(noise)
            grad_ori.clamp_(qmin,qmax).round_() # quatnize

            grad_ori.add_(-qmin).mul_(scale).add_(min_value) # dequantize

        grad_input = (grad_output.clone() * grad_threshold) + (grad_ori.clone() * (1 - grad_threshold))
...
class grad_quant_DW(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, num_bits = 8, init=True, clip_ratio=1, grad_max=1):
        ctx.num_bits = num_bits
        ctx.init = init
    ...
    def backward(ctx, grad_output):
        if ctx.num_bits != 32:
            ...
            grad_max = grad_output.permute(1,0,2,3).reshape(c, b*h*w).abs().max(dim=1)[0]
            grad_max = grad_max.view(1,c,1,1)

            alpha = 0.8

            # alpha = ctx.clip_ratio
            # alpha = alpha.view(1,c,1,1)

            min_value = -grad_max * alpha
            max_value = grad_max * alpha

            if (min_value == 0).sum() >= 1:
                min_value = min_value - 1e-8
                max_value = max_value + 1e-8
...
def quantize_grad_DW(x, num_bits, init, clip_ratio, grad_max):
    return grad_quant_DW().apply(x, num_bits, init, clip_ratio, grad_max)
...
class DSQConv_a_mobile_DW(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,
                momentum = 0.1, num_bit = 3, QInput = True, bSetQ = True, layer_num=0, symmetric=False):
        super(DSQConv_a_mobile_DW, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
        self.num_bit = num_bit
        self.quan_input = QInput
        self.bit_range = 2**self.num_bit -1
        self.is_quan = bSetQ
    ...
    def forward(self, x):
        ...
        if self.is_quan:
            ...
            moment = 1
            if bit_grad <= 16:
                output = quantize_grad_DW(output, bit_grad, self.init, self.clip_ratio, self.grad_max)
...
class DSQConv_a_mobile_DW_1(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,
                momentum = 0.1, num_bit = 3, QInput = True, bSetQ = True, layer_num=0, symmetric=False):
        super(DSQConv_a_mobile_DW_1, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
        self.num_bit = num_bit
        self.quan_input = QInput
        self.bit_range = 2**self.num_bit -1
        self.is_quan = bSetQ
        ...
        if self.is_quan:
            ...
            self.sW = nn.Parameter(data = torch.tensor(8).float())
            self.register_buffer('init', torch.tensor(1.0).float())
            self.register_buffer('clip_ratio', torch.ones(out_channels).float())
            self.register_buffer('grad_max', torch.tensor(-1.0).float())
            # self.beta = nn.Parameter(data = torch.tensor(0.2).float())
...
class DSQConv_a(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,
        ...
        if self.is_quan:
            ...
            self.sW = nn.Parameter(data = torch.tensor(8).float())
            self.register_buffer('init', torch.tensor(1.0).float())
            self.register_buffer('symmetric', torch.tensor(1.0).float())
    ...
    def forward(self, x):
        ...
        if self.is_quan:
            ...
            if self.init == 1:
                ...
                if symmetric > 0:
                    # self.symmetric = True
                    self.symmetric.data.fill_(1)
                else:
                    self.symmetric.data.fill_(0)
                    # self.symmetric = False
