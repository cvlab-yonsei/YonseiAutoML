...
class Quantizer_a_scale_ver1(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, bit, Q_diff):
        s = (2 ** bit) - 1
        ...
        ctx.save_for_backward(diff, s, bit, Q_diff)

        return out_r / s
    @staticmethod
    def backward(ctx, g):
        diff, s, bit, Q_diff = ctx.saved_tensors
        ...
        g_mask = (torch.sign(g) == torch.sign(Q_diff)).float()

        # g_scale = (1 + (0.1 * torch.sign(g) * Q_diff))
        g_scale = torch.exp(torch.sign(g) * Q_diff * g_mask)

        return g * g_scale, grad_b_norm * g, None

class Quantizer_w_scale_ver1(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, bit, Q_diff):
        if bit == 1:
            s = (2 ** bit) - 1
...
class DSQConv_a(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,
        ...
        self.bit_w_grad = torch.tensor(0.)
        self.bit_a_grad = torch.tensor(0.)
    ...
    def round_bit_a(self, x, bit):
        ...
        return out
    def quantizer(self, x, bit, bit_sig):
        bit_range = (2 ** bit) - 1

        bit_target = bit + bit_sig

        if bit_target < 1:
            bit_target = 1

        bit_diff = (torch.abs(bit_target - self.bit_w) - 0.5).abs()

        Q_diff = -(self.round_bit_w(x, bit.detach()) - self.round_bit_w(x, (bit_target)))
        Q_diff = Q_diff * torch.exp(-bit_diff/(10*self.bit_w_grad.abs() + 1e-8))

        # Q_diff = Q_diff * 1000
        # Q_diff = self.bit_w_grad.abs() * Q_diff

        # out = Quantizer_w.apply(x, bit)
        out = Quantizer_w_scale_ver1.apply(x, bit, Q_diff.detach())
        out = 2 * out - 1
        return out

    def quantizer_a(self, x, bit, bit_sig):
        bit_range = (2 ** bit) - 1
        ...
        bit_diff = (torch.abs(bit_target - self.bit_a) - 0.5).abs()

        Q_diff = -(self.round_bit_a(x, bit.detach()) - self.round_bit_a(x, (bit_target)))
        Q_diff = Q_diff * torch.exp(-bit_diff/(10*self.bit_a_grad.abs() + 1e-8))

        # Q_diff = Q_diff * 1000
        # Q_diff = self.bit_a_grad.abs() * Q_diff

        # out = Quantizer_a.apply(x, bit)
        out = Quantizer_a_scale_ver1.apply(x, bit, Q_diff.detach())
        return out
    ...
    def w_quan(self, x, s):
        ...
        out = torch.clamp(out, min=0, max=1)
        out_r = self.quantizer(out, bit_w, torch.sign(self.bit_w_grad))
    ...
    def a_quan(self, x, u, l):
        ...
        output = self.quantizer_a(out, bit_a, torch.sign(self.bit_a_grad))
