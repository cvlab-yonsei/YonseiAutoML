...
class grad_quant(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        if ctx.num_bits != 32:
            ...
            # alpha = 0.4
            # alpha_res = 1 - alpha
            # alpha_noise = torch.distributions.Uniform(0, alpha_res.item() + 1e-6).sample()
            # alpha = alpha + alpha_noise
            # if ctx.grad_max < 0:
            #     grad_max = grad_output.abs().max()
            # else:
            #     grad_max = ctx.grad_max
            # ori_scale = grad_output.abs().mean()
            grad_max = grad_output.abs().max()
...
class DSQConv_a(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,
        ...
        self.output_grad_scale = torch.tensor(0.0)
        self.quant_output_grad_scale = torch.tensor(0.0)
    ...
    def forward(self, x):
        ...
        if self.is_quan:
            ...
            # self.fp_weight = self.weight
            # self.q_weight = Qweight
...
class DSQLinear(nn.Linear):
    ...
    def forward(self, x):
        ...
        if self.is_quan:
            ...
            if self.quan_input:
                curr_running_la = -self.uA
                curr_running_ua = self.uA

            Qweight = self.w_quan(self.weight, self.sW) * self.sW

            if self.bias_T:
                Qbias = self.w_quan(self.bias, self.sB) * torch.abs(self.betaB)
            else:
                Qbias = self.bias
...
#             self.quant_output_buff = output
#             self.quant_output_buff.cuda()
#             self.quant_output_buff.requires_grad = True
#             if self.training:
#                 self.quant_output_buff.retain_grad()
            
#             moment = 1

#             # output = quantize_grad(self.quant_output_buff, bit_grad, self.init, self.clip_ratio, moment)
#             output = self.quant_output_buff

#             if self.init == 1:
#                 self.init = torch.tensor(0)

#             self.output_buff = output
#             self.output_buff.cuda()
#             self.output_buff.requires_grad = True
#             if self.training:
#                 self.output_buff.retain_grad()
#                 self.pre_weight = self.weight.clone()
#             # output = F.conv2d(Qactivation, Qweight, Qbias, self.stride, self.padding, self.dilation, self.groups)
#             output = torch.abs(self.beta) * self.output_buff
