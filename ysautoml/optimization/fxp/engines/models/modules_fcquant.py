...
class DSQConv_a(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,
        ...
        self.bit_w_transition = torch.tensor(0)
        self.bit_a_transition = torch.tensor(0)
    ...
    def forward(self, x):
        ...
        if self.is_quan:
            ...
            if self.training:
                if self.bit_w_score > 1e-3:
                    self.alpha_w_gauge = self.alpha_w_gauge + (1 / 135150)
                    curr_bit_w_transtion = (self.bit_w.floor() + self.bit_w.ceil()) / 2
                    
                    if self.bit_w_transition.item() != curr_bit_w_transtion.item():
                        self.bit_w_transition = curr_bit_w_transtion
                        self.alpha_w = 0
                    
                else:
                    self.alpha_w_gauge = torch.tensor(0.)
                
                if self.bit_a_score > 1e-3:
                    self.alpha_a_gauge = self.alpha_a_gauge + (1 / 135150)
                    curr_bit_a_transition = (self.bit_a.floor() + self.bit_a.ceil()) / 2

                    if self.bit_a_transition.item() != curr_bit_a_transition.item():
                        self.bit_a_transition = curr_bit_a_transition
                        self.alpha_a = 0
            ...
            if self.alpha_w_gauge > gamma:
                self.alpha_w = 1
            # else:
            #     self.alpha_w = 0

            if self.alpha_a_gauge > gamma:
                self.alpha_a = 1
            # else:
            #     self.alpha_a = 0
            ...
            if self.update_w:
                ...
                if self.bit_w.round() != self.pre_bit_w.round():
                    ...
                    if direction < 0 and w_diff_grad:
                        self.bit_w.data = self.bit_w - self.alpha_w * (self.bit_w - self.pre_bit_w)
            ...
            if self.update_a:
                ...
                if self.bit_a.round() != self.pre_bit_a.round():
                    ...
                    if direction < 0 and a_diff_grad:
                        self.bit_a.data = self.bit_a - self.alpha_a * (self.bit_a - self.pre_bit_a)
