...
class TinyNetworkRANDOM(nn.Module):
    def __init__(
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(3, C, kernel_size=3, padding=1, bias=False), nn.BatchNorm2d(C, affine = affine, track_running_stats = track_running_stats)
        )
        ...
        for index, (C_curr, reduction) in enumerate(
            ...
            if reduction:
                cell = ResNetBasicblock(C_prev, C_curr, 2, affine = affine, track_running_stats = track_running_stats)
            else:
                cell = SearchCell(
                    C_prev,
                    C_curr,
